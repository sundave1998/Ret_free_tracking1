function [X,avg_x,Err, dist] = Mod_Ret_free_tracking1(A, W,stepsize,multistep,X0, maxiter,xopt)
N = size(W,1);
[n, r] = size(X0{1});
...
for k = 1:maxiter
    ...
    %     max_y = 0;
    for i = 1:N
        new_d = proj_tangent(X{i},0.5*c{i}) + y{i};
        %         [U,~,V] = svd(X{i} + new_d,0);
        %         X{i} = U*V';
        % X{i} = X{i} + new_d - p{i};
        X{i} = X{i} + new_d;
